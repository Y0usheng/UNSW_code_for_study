...
struct sync_counter *counter_initialise(int val)
{
        ...
        sc_ptr->lock = lock_create("counter_lock");
        if (sc_ptr->lock == NULL)
        {
                kfree(sc_ptr); // Clean up previously allocated memory if lock creation fails.
                return NULL;
        }
...
{
        int count;
        lock_acquire(sc_ptr->lock); // Ensure exclusive access to read.
         ...
         * **********************************************************************
         */
        lock_release(sc_ptr->lock); // Release lock before destroying it.
        lock_destroy(sc_ptr->lock); // Destroy the lock.
        kfree(sc_ptr);
        return count;
}
...
{
        lock_acquire(sc_ptr->lock);
        sc_ptr->counter = sc_ptr->counter + 1;
        lock_release(sc_ptr->lock);
}
